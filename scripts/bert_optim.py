        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if self.defaults['max_grad_norm'] > 0:
                    clip_grad_norm_(p, self.defaults['max_grad_norm'])
                ...
                if self.defaults['t_total'] != -1:
                    schedule_fct = schedule_func(self.defaults['schedule'])
                    lr_scheduled = self.defaults['lr'] * schedule_fct(state['step']/self.defaults['t_total'], self.defaults['warmup'])
                else:
                    lr_scheduled = self.defaults['lr']

...
from modeling import BertForMemory
from modified_sampler import ModifiedDistributedSampler
...
MODEL_CLASSES = {
    ...
    'memory': (BertConfig, BertForMemory, BertTokenizer),
...
def train(args, train_dataset, model, tokenizer):
    ...
    train_sampler = SequentialSampler(
        train_dataset) if args.local_rank == -1 else ModifiedDistributedSampler(train_dataset, shuffle=False)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    eval_sampler = SequentialSampler(
        dataset) if args.local_rank == -1 else ModifiedDistributedSampler(dataset, shuffle=False)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        # take ground truth for end
        if args.model_type == "memory":
            end_positions = batch[7]
            end_logits = torch.zeros(start_logits.shape, device=start_logits.device, dtype=end_positions.dtype)
            end_logits[torch.arange(end_logits.shape[0]), end_positions] = 1

        # # take ground truth for start
        # if args.model_type == "memory":
        #     start_positions = batch[6]
        #     start_logits = torch.zeros(end_logits.shape, device=end_logits.device, dtype=end_logits.dtype)
        #     start_logits[torch.arange(start_logits.shape[0]), start_positions] = 1
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if evaluate:
        ...
        try:
            ...
            all_start_positions = None
            all_end_positions = None
